...
Point Secp256K1::ComputePublicKey(Int* privKey) // hiiu - here -codenow
{
...
std::string Secp256K1::GetAddressETH(unsigned char* hash) //hiiu - here - codenow
{
	char tmp[3];
	std::string ret;
...
std::string Secp256K1::GetAddressETH(Point& pubKey) // hiiu - here - codenow
{
	uint32_t hash[5];
	char tmp[3];
	std::string ret;
